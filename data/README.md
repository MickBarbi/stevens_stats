...
| File | Contents |
|---|---|
| `athletes.json` | One entry per athlete. `first_name`, `last_name`, `year`, `sex`, `active` come from the roster scrape. `nickname`, `bio`, `image_path`, `graduation_year`, `awards`, `status`, `year_override` are **preserved** across re-scrapes — edit those here. `year_override` (int) pins the class year when TFRRS is wrong. `"active": false` = a former athlete (dropped off the roster, or an alumnus backfilled via `scraper/rosters.txt`); they get an `/athlete` page and Top 10 links but are hidden from the roster / events / feed. |
